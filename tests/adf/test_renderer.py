...
def test_paragraph():
    doc = blocks.Document(children=[
        blocks.Paragraph(children=[inlines.Text(text="hello")])
    ])
...
def test_heading():
    doc = blocks.Document(children=[
        blocks.Heading(level=2, children=[inlines.Text(text="title")])
    ])
    result = render(doc)
...
def test_table():
    doc = blocks.Document(children=[
        blocks.Table(
            head=[blocks.TableHeader(children=[
                blocks.Paragraph(children=[inlines.Text(text="H")])
            ])],
            body=[[blocks.TableCell(children=[
...
def test_emphasis():
    doc = blocks.Document(children=[
        blocks.Paragraph(children=[
            inlines.Emphasis(children=[inlines.Text(text="em")])
        ])
    ])
    result = render(doc)
...
def test_strikethrough():
    doc = blocks.Document(children=[
        blocks.Paragraph(children=[
            inlines.Strikethrough(children=[inlines.Text(text="del")])
        ])
    ])
    result = render(doc)
...
def test_link_with_title():
    doc = blocks.Document(children=[
        blocks.Paragraph(children=[
            inlines.Link(url="http://x", children=[inlines.Text(text="link")], title="t")
        ])
    ])
    result = render(doc)
    marks = result["content"][0]["content"][0]["marks"]
...
def test_code_span():
    doc = blocks.Document(children=[
        blocks.Paragraph(children=[inlines.CodeSpan(code="x")])
    ])
    result = render(doc)
    assert result["content"][0]["content"] == [
...
def test_soft_break():
    doc = blocks.Document(children=[
        blocks.Paragraph(children=[inlines.SoftBreak()])
    ])
    result = render(doc)
...
def test_image_as_media_single():
    ...
    doc = blocks.Document(children=[
        blocks.Paragraph(children=[inlines.Image(url="http://img.png")])
    ])
    result = render(doc)
...
def test_nested_marks_ordering():
    ...
    doc = blocks.Document(children=[
        blocks.Paragraph(children=[
            inlines.Strong(children=[
                inlines.Emphasis(children=[inlines.Text(text="bi")])
            ])
        ])
    ])
    result = render(doc)
...
def test_expand_no_title():
    doc = blocks.Document(children=[
        blocks.Expand(children=[blocks.Paragraph(children=[])])
    ])
...
def test_task_list():
    doc = blocks.Document(children=[
        blocks.TaskList(items=[
            blocks.TaskItem(children=[inlines.Text(text="task")], state="TODO", local_id="id-1"),
        ])
    ])
    result = render(doc)
    tl = result["content"][0]
...
def test_decision_list():
    doc = blocks.Document(children=[
        blocks.DecisionList(items=[
            blocks.DecisionItem(children=[inlines.Text(text="decide")], state="DECIDED", local_id="d-1"),
        ])
    ])
    result = render(doc)
    dl = result["content"][0]
...
def test_layout_section():
    doc = blocks.Document(children=[
        blocks.LayoutSection(columns=[
            blocks.LayoutColumn(
                children=[blocks.Paragraph(children=[inlines.Text(text="left")])],
                width=50.0,
            ),
            blocks.LayoutColumn(
                children=[blocks.Paragraph(children=[inlines.Text(text="right")])],
                width=50.0,
            ),
        ])
    ])
...
def test_media_single_no_attrs():
    doc = blocks.Document(children=[
        blocks.MediaSingle(media=blocks.Media(media_type="external", url="http://img.png"))
    ])
    assert "attrs" not in render(doc)["content"][0]
...
def test_block_card():
    doc = blocks.Document(children=[
        blocks.BlockCard(url="http://card")
    ])
    result = render(doc)
    assert result["content"][0] == {"type": "blockCard", "attrs": {"url": "http://card"}}


def test_embed_card():
    doc = blocks.Document(children=[
        blocks.EmbedCard(url="http://embed", layout="wide", width=100.0)
    ])
    result = render(doc)
    ec = result["content"][0]
...
def test_extension_raw():
    raw: dict[str, Any] = {"type": "extension", "attrs": {"extensionType": "t", "extensionKey": "k"}}
    doc = blocks.Document(children=[blocks.Extension(raw=raw)])
    assert render(doc)["content"][0] is raw


def test_bodied_extension_raw():
    raw: dict[str, Any] = {"type": "bodiedExtension", "attrs": {"extensionType": "t", "extensionKey": "k"}}
    doc = blocks.Document(children=[blocks.BodiedExtension(raw=raw)])
    assert render(doc)["content"][0] is raw


def test_sync_block_raw():
    raw: dict[str, Any] = {"type": "syncBlock", "attrs": {"resourceId": "r", "localId": "l"}}
...
def test_emoji():
    doc = blocks.Document(children=[
        blocks.Paragraph(children=[
            inlines.Emoji(short_name=":smile:", id="e-1")
        ])
    ])
    result = render(doc)
    assert result["content"][0]["content"][0] == {
...
def test_date():
    doc = blocks.Document(children=[
        blocks.Paragraph(children=[inlines.Date(timestamp="1234567890")])
    ])
    result = render(doc)
    assert result["content"][0]["content"][0] == {
        "type": "date",
...
def test_inline_card():
    doc = blocks.Document(children=[
        blocks.Paragraph(children=[
            inlines.InlineCard(url="http://inline-card")
        ])
    ])
    result = render(doc)
    assert result["content"][0]["content"][0] == {
...
def test_placeholder():
    doc = blocks.Document(children=[
        blocks.Paragraph(children=[inlines.Placeholder(text="Type here")])
    ])
    result = render(doc)
    assert result["content"][0]["content"][0] == {
        "type": "placeholder",
...
def test_inline_extension_raw():
    ...
    doc = blocks.Document(children=[
        blocks.Paragraph(children=[inlines.InlineExtension(raw=raw)])
    ])
...
def test_underline():
    doc = blocks.Document(children=[
        blocks.Paragraph(children=[
            inlines.Underline(children=[inlines.Text(text="u")])
        ])
    ])
    result = render(doc)
...
def test_text_color():
    doc = blocks.Document(children=[
        blocks.Paragraph(children=[
            inlines.TextColor(color="#ff0000", children=[inlines.Text(text="red")])
        ])
    ])
    result = render(doc)
...
def test_background_color():
    doc = blocks.Document(children=[
        blocks.Paragraph(children=[
            inlines.BackgroundColor(color="#00ff00", children=[inlines.Text(text="bg")])
        ])
    ])
    result = render(doc)
    assert result["content"][0]["content"][0]["marks"] == [
...
def test_subsup():
    doc = blocks.Document(children=[
        blocks.Paragraph(children=[
            inlines.SubSup(type="sub", children=[inlines.Text(text="2")])
        ])
    ])
    result = render(doc)
    assert result["content"][0]["content"][0]["marks"] == [
...
def test_annotation():
    doc = blocks.Document(children=[
        blocks.Paragraph(children=[
            inlines.Annotation(id="ann-1", children=[inlines.Text(text="noted")])
        ])
    ])
    result = render(doc)
    assert result["content"][0]["content"][0]["marks"] == [
...
def test_paragraph_alignment():
    doc = blocks.Document(children=[
        blocks.Paragraph(children=[inlines.Text(text="centered")], alignment="center")
    ])
    result = render(doc)
...
def test_heading_indentation():
    doc = blocks.Document(children=[
        blocks.Heading(level=1, children=[inlines.Text(text="indented")], indentation=2)
    ])
    result = render(doc)
    assert result["content"][0]["marks"] == [
...
def test_paragraph_no_marks():
    ...
    doc = blocks.Document(children=[
        blocks.Paragraph(children=[inlines.Text(text="plain")])
    ])
    result = render(doc)
...
def _strip_local_ids(obj: dict[str, Any]) -> dict[str, Any]:
    ...
    for k, v in obj.items():
        ...
        if isinstance(v, dict):
            ...
            result[k] = [
                _strip_local_ids(cast(dict[str, Any], item)) if isinstance(item, dict) else item
...
def test_roundtrip_link():
    adf: dict[str, Any] = _doc(_p(
        _text("link", marks=[{"type": "link", "attrs": {"href": "http://x"}}])
    ))
    assert render(parse(adf)) == adf
...
def test_roundtrip_table_with_attrs():
    adf: dict[str, Any] = _doc({
        ...
        "content": [
            {"type": "tableRow", "content": [
                {"type": "tableCell", "content": [_p(_text("x"))],
...
def test_roundtrip_media_single():
    adf: dict[str, Any] = _doc({
        "type": "mediaSingle",
        "content": [{"type": "media", "attrs": {"type": "external", "url": "http://img.png"}}],
        "attrs": {"layout": "center"},
    })
    assert render(parse(adf)) == adf
...
def test_roundtrip_media_group():
    adf: dict[str, Any] = _doc({
        "type": "mediaGroup",
        "content": [
            {"type": "media", "attrs": {"type": "file", "id": "f-1", "collection": "c"}},
        ],
    })
    assert render(parse(adf)) == adf
...
def test_roundtrip_text_color():
    adf: dict[str, Any] = _doc(_p(
        _text("red", marks=[{"type": "textColor", "attrs": {"color": "#ff0000"}}])
    ))
    assert render(parse(adf)) == adf
...
def test_roundtrip_background_color():
    adf: dict[str, Any] = _doc(_p(
        _text("bg", marks=[{"type": "backgroundColor", "attrs": {"color": "#00ff00"}}])
    ))
    assert render(parse(adf)) == adf
...
def test_table_column_headers():
    ...
    doc = blocks.Document(children=[
        blocks.Table(
            ...
            body=[[blocks.TableHeader(children=[
                blocks.Paragraph(children=[inlines.Text(text="Col")])
            ])], [blocks.TableCell(children=[
                blocks.Paragraph(children=[inlines.Text(text="D")])
            ])]],
        )
    ])
